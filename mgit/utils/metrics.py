...
def normalized_center_error(rects1, rects2, bound):
    ...
    for i in range(len(rects1)):
        ...
        thr_max = max((thr_ul[i] + calculate_detla((0, 0), rects2[i])[0]),
                      (thr_ur[i] + calculate_detla((width, 0), rects2[i])[0]),
                      (thr_ll[i] + calculate_detla((0, height), rects2[i])[0]),
                      (thr_lr[i] + calculate_detla((width, height), rects2[i])[0]))
