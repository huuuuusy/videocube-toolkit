...
class ExperimentVOT(object):
    ...
    """
    def __init__(self, root_dir, save_dir, repetition,  version=2017, read_image=True, list_file=None,
    ...
    def run(self, tracker, visualize, save_img, method):
        ...
        if 'unsupervised' in self.experiments:
            self.run_unsupervised(tracker, visualize, save_img, method)
    ...
    def run_unsupervised(self, tracker, visualize, save_img, method):
        print('Running unsupervised experiment...')

        # loop over the complete dataset
        for s, (img_files, anno, restart_flag, _) in enumerate(self.dataset):
            seq_name = self.dataset.seq_names[s]
            print('--Sequence %d/%d: %s' % (s + 1, len(self.dataset), seq_name))
            ...
            if method == None:
                ...
                boxes, times = tracker.track(seq_name, img_files, anno_rects, restart_flag, visualize, seq_result_dir, save_img, method)
            elif method == 'restart':
                # tracking in novel R-OPE mechanism
                boxes, times, init_positions = tracker.track(seq_name, img_files, anno_rects,  restart_flag, visualize, seq_result_dir, save_img, method)
