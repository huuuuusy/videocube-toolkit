...
class OTB(object):
    ...
    def __init__(self, root_dir, version=2015, download=False):
        ...
        self.restart_files = sorted(list(chain.from_iterable(glob.glob(
            os.path.join(root_dir, s, 'restart.txt')) for s in valid_seqs)))
        # remove empty annotation files
        # (e.g., groundtruth_rect.1.txt of Human4)
        self.anno_files = self._filter_files(self.anno_files)
        self.restart_files = self._filter_files(self.restart_files)
